...
const QAOS101 = () => {
  ...
  const handleSubmission = async (index) => {
    ...
    try {
      const response = await fetch('https://nxb4401.uta.cloud/php/QAOupdateMarks.php', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          ...
          UserID: studentID,
          QAOResult: selectedValue.value === 'Yes' ? 'Pass' : 'Fail',
        }),
      });
      ...
      if (response.ok) {
        ...
        setError('Failed to update QAOResult');
      }
    } catch (error) {
      setError('Error updating QAOResult');
      console.error('Error updating QAOResult:', error);
    }
  };
                    ...
                    <tr key={course.CourseID}>
                      ...
                      <td>{course.Given}</td>
                        <td>
                            {course.Given === 'No' ? (
                              '-'
                            ) : course.Given === 'Yes' && course.QAOResult === null ? (
                            ...
                            ) : course.QAOResult === 'Fail' ? (
                              <span style={{ color: 'red' }}>✕</span>
                            ) : (
                              '-'
                            )}
                          </td>
                      <td>{course.Given === 'No' && course.QAOResult === null ? '-' : course.Marks}</td>
                      <td>
                        {course.Given === 'No' ? (
                          '-'
                        ) : course.QAOResult === 'Pass' ? (
